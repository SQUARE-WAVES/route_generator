...
suite('test the schema generators', function () {
	test('unparameterized route', function () {
		...
		assert.equal(gen(), schema.path, 'the schema should match the path it was generated with');
	});
	...
	test('parameterized route without options', function () {
		...
		assert.equal(gen({'name':'charles'}), '/hello/charles', 'the schema should match a correct instance');
	...
	test('parameterized route with optional components', function () {
		...
		assert.equal(gen({'title':'doctor', 'name':'barnabus'}), '/hello/doctor/barnabus', 'generator should work with optional params filled in');
		assert.equal(gen({'name':'charles'}), '/hello/charles', 'generator should work without optional params filled in');
	...
	test('parameterized route with optional splat', function () {
		...
		assert.equal('/hello/charles/a/b/c/d', gen({'name':'charles','extra':'a/b/c/d'}), 'the gen should work with splat included');
		assert.equal('/hello/barnabus', gen({'name':'barnabus'}), 'a missing optional splat is fine');
	})
});